"""
STLE v3 Visualization Generator
Creates visual demonstrations of evidence-scaled Posterior Networks concepts

Generates 4 publication-quality plots:
  1. Decision boundary + accessibility heatmap + frontier regions
  2. In-distribution vs out-of-distribution accessibility
  3. Epistemic vs aleatoric uncertainty decomposition
  4. Complementarity verification (μ_x + μ_y = 1)
"""

import numpy as np
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
from stle_v3_minimal_demo import MinimalSTLEv3, generate_moons_data, generate_circles_data


def create_decision_boundary_plot(model, X_train, y_train, X_test, save_path):
    """
    Visualize decision boundary with v3 accessibility coloring
    """
    ...
    # Plot 2: Accessibility μ_x (v3 formula)
    ...
    axes[1].set_title('Accessibility μ_x (v3: Evidence-Scaled)', fontsize=14, fontweight='bold')
    ...
    frontier_mask = (grid_pred['mu_x'] > 0.3) & (grid_pred['mu_x'] < 0.7)
    accessible_mask = grid_pred['mu_x'] >= 0.7
    inaccessible_mask = grid_pred['mu_x'] <= 0.3
    
    Z_frontier = np.zeros_like(Z_mu_x)
    Z_frontier[accessible_mask.reshape(xx.shape)] = 2
    Z_frontier[frontier_mask.reshape(xx.shape)] = 1
    Z_frontier[inaccessible_mask.reshape(xx.shape)] = 0
    ...
    red_patch = mpatches.Patch(color='#ff6b6b', label='Inaccessible (μ_x < 0.3)')
    yellow_patch = mpatches.Patch(color='#ffd93d', label='Frontier (0.3 ≤ μ_x < 0.7)')
    green_patch = mpatches.Patch(color='#6bcf7f', label='Accessible (μ_x ≥ 0.7)')
...
def create_ood_comparison_plot(model, X_id, X_ood, save_path):
    ...
    axes[1].hist(pred_id['mu_x'], bins=30, alpha=0.6, label='In-Distribution',
                color='blue', edgecolor='black')
    axes[1].hist(pred_ood['mu_x'], bins=30, alpha=0.6, label='Out-of-Distribution',
    ...
    axes[1].set_title('Accessibility Distribution (v3)', fontsize=14, fontweight='bold')
...
def create_uncertainty_plot(model, X_test, y_test, save_path):
    ...
    axes[0].set_title('Epistemic Uncertainty\n(Reducible with more data)', fontsize=12, fontweight='bold')
    axes[0].set_xlabel('Feature 1')
    axes[0].set_ylabel('Feature 2')
    ...
    axes[1].set_title('Aleatoric Uncertainty\n(Irreducible randomness)', fontsize=12, fontweight='bold')
    axes[1].set_xlabel('Feature 1')
    axes[1].set_ylabel('Feature 2')
    plt.colorbar(scatter2, ax=axes[1], label='Aleatoric')
    
    # Plot 3: Scatter — epistemic vs aleatoric
...
def create_complementarity_plot(model, X_test, save_path):
    ...
    axes[0].scatter(pred['mu_x'], pred['mu_y'], alpha=0.6, s=50,
                   edgecolors='k', linewidths=0.5)
...
def generate_all_visualizations():
    """Generate complete set of STLE v3 visualizations"""
    print("\n" + "=" * 60)
    print("Generating STLE v3 Visualizations")
    print("=" * 60 + "\n")
    ...
    create_decision_boundary_plot(
        ...
        '/mnt/user-data/outputs/stle_v3_decision_boundary.png'
    )
    
    create_ood_comparison_plot(
        model, X_test, X_ood,
        '/mnt/user-data/outputs/stle_v3_ood_comparison.png'
    )
    
    create_uncertainty_plot(
        model, X_test, y_test,
        '/mnt/user-data/outputs/stle_v3_uncertainty_decomposition.png'
    )
    
    create_complementarity_plot(
        model, X_test,
        '/mnt/user-data/outputs/stle_v3_complementarity.png'
    )
    
    print("\n" + "=" * 60)
    print("All STLE v3 visualizations generated successfully!")
    print("=" * 60)
